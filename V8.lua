...
menu.divider(pvphelp, KER_LANG_TABLE[46])

menuToggleLoop(pvphelp, KER_LANG_TABLE[47], {"silentaim", "saimbot"}, "A silent aimbot with bone selection.", function ()
    local ourped = GetLocalPed()
    if PED.IS_PED_SHOOTING(ourped) then
        ...
        --     end
        -- end
        -- (( OLD TARGETTING USED MULTIPLE TARGETS, WITH INRANGE[i] ))
        local targetplayer = GetClosestPlayerWithRange_Whitelist(10000)
        local weaponHash = 177293209 --heavy sniper mk2 hash
        local bulletSpeed = 1000
        --util.toast("Entities in range of // " .. AIM_Dist .. " // :" .. #inRange)
        --for i = 1, #inRange do
            local coord = getEntityCoords(targetplayer)
            if (PED.IS_PED_A_PLAYER(targetplayer) and not AIM_NPCS) or (not PED.IS_PED_A_PLAYER(targetplayer) and AIM_NPCS) then --check if player
                if not PED.IS_PED_DEAD_OR_DYING(targetplayer, 1) then --check for dead/dying
                    if (ENTITY.HAS_ENTITY_CLEAR_LOS_TO_ENTITY(ourped, targetplayer, 17) and LOS_CHECK == true) or (LOS_CHECK == false) then --check if we have line of sight
                        if (PED.IS_PED_FACING_PED(ourped, targetplayer, AIM_FOV) and FOV_CHECK == true) or (FOV_CHECK == false) then --check for FOV
                            if not AIM_WHITELIST[NETWORK.NETWORK_GET_PLAYER_INDEX_FROM_PED(targetplayer)] then --check if PID doesn't match the whitelist
                                if DistanceBetweenTwoCoords(coord, getEntityCoords(ourped)) < 401 and AIM_LEGITSILENT and ENTITY.HAS_ENTITY_CLEAR_LOS_TO_ENTITY(ourped, targetplayer, 17) then --check if they're less than 401 meters away (hitscan), and in LOS
                                    --shooting done here, we have all preloads
                                    local playerID = NETWORK.NETWORK_GET_PLAYER_INDEX_FROM_PED(targetplayer)
                                    local playerName = NETWORK.NETWORK_PLAYER_GET_NAME(playerID)
                                    local pveh = PED.GET_VEHICLE_PED_IS_IN(targetplayer, false)
                                    if SE_Notifications then
                                    ...
                                    if AIM_Head then
                                        local bonec = PED.GET_PED_BONE_COORDS(targetplayer, 12844, 0, 0, 0)
                                    ...
                                    elseif not AIM_HEAD and AIM_HEADVEH and PED.IS_PED_IN_ANY_VEHICLE(targetplayer, false) then --check for "target head if target in is vehicle"
                                    ...
                                    if AIM_Spine2 then
                                        ...
                                        local bonec = PED.GET_PED_BONE_COORDS(targetplayer, 24817, 0, 0, 0)
                                        MISC.SHOOT_SINGLE_BULLET_BETWEEN_COORDS_IGNORE_ENTITY(forwardOffset.x, forwardOffset.y, forwardOffset.z, bonec.x, bonec.y, bonec.z, AIM_DMG, true, weaponHash, GetLocalPed(), true, false, bulletSpeed, pveh, true)
                                    end
                                    if AIM_Pelvis then
                                        local bonec = PED.GET_PED_BONE_COORDS(targetplayer, 11816, 0, 0, 0)
                                        MISC.SHOOT_SINGLE_BULLET_BETWEEN_COORDS_IGNORE_ENTITY(forwardOffset.x, forwardOffset.y, forwardOffset.z, bonec.x, bonec.y, bonec.z, AIM_DMG, true, weaponHash, GetLocalPed(), true, false, bulletSpeed, pveh, true)
                                    end
                                    if AIM_Toe0 then
                                        local bonec = PED.GET_PED_BONE_COORDS(targetplayer, 20781, 0, 0, 0)
                                        MISC.SHOOT_SINGLE_BULLET_BETWEEN_COORDS_IGNORE_ENTITY(forwardOffset.x, forwardOffset.y, forwardOffset.z, bonec.x, bonec.y, bonec.z, AIM_DMG, true, weaponHash, GetLocalPed(), true, false, bulletSpeed, pveh, true)
                                    end
                                    if AIM_RHand then
                                        local bonec = PED.GET_PED_BONE_COORDS(targetplayer, 6286, 0, 0, 0)
                                        MISC.SHOOT_SINGLE_BULLET_BETWEEN_COORDS_IGNORE_ENTITY(forwardOffset.x, forwardOffset.y, forwardOffset.z, bonec.x, bonec.y, bonec.z, AIM_DMG, true, weaponHash, GetLocalPed(), true, false, bulletSpeed, pveh, true)
                                    end
                                else
                                    --shooting done here, we have all preloads
                                    ...
                                    local forwardOffset = ENTITY.GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(targetplayer, 0, 1, 1)
                                    if AIM_Head then
                                        local bonec = PED.GET_PED_BONE_COORDS(targetplayer, 12844, 0, 0, 0)
                                        MISC.SHOOT_SINGLE_BULLET_BETWEEN_COORDS_IGNORE_ENTITY(forwardOffset.x, forwardOffset.y, forwardOffset.z, bonec.x, bonec.y, bonec.z, AIM_DMG, true, weaponHash, GetLocalPed(), true, false, bulletSpeed, pveh, true)
                                    elseif not AIM_HEAD and AIM_HEADVEH and PED.IS_PED_IN_ANY_VEHICLE(targetplayer, false) then
                                        util.toast("VehChecked " .. tostring(playerName))
                                        local bonec = PED.GET_PED_BONE_COORDS(targetplayer, 12844, 0, 0, 0)
                                        MISC.SHOOT_SINGLE_BULLET_BETWEEN_COORDS_IGNORE_ENTITY(forwardOffset.x, forwardOffset.y, forwardOffset.z, bonec.x, bonec.y, bonec.z, AIM_DMG, true, weaponHash, GetLocalPed(), true, false, bulletSpeed, pveh, true)
                                    end
...
local silentAimSettings = menu.list(pvphelp, KER_LANG_TABLE[48], {}, "")

menu.slider(silentAimSettings, KER_LANG_TABLE[49], {"silentaimdamage", "silentdamage", "saimdamage"}, "The amount of damage Silent Aimbot does. Not accurate, sadly...", 1, 10000, 30, 10, function(value)
    AIM_DMG = value
end)

menu.slider(silentAimSettings, KER_LANG_TABLE[50], {"silentaimrange", "silentrange", "saimrange"}, "Silent Aimbot Range", 1, 10000, 300, 1, function (value)
    AIM_Dist = value
end)

menu.slider(silentAimSettings, KER_LANG_TABLE[51], {"silentaimfov", "silentfov", "saimfov"}, "The FOV of which players can be targeted. (divided by 10)", 1, 2700, 1, 10, function (value)
    AIM_FOV = value / 10
end)

menuToggle(silentAimSettings, KER_LANG_TABLE[52], {"silentaimvehicle", "silentvehice", "saveh"}, "Removes line-of-sight checks. Done to make silent aim work for vehicles. Please do note that the FOV is taken FROM THE VEHICLE, NOT FROM WHERE YOU ARE FACING.", function (on)
    LOS_CHECK = not on
end)

menuToggle(silentAimSettings, KER_LANG_TABLE[53], {"silentlegit"}, "If you have Line-of-Sight, attempts to shoot a bullet from you to the player. Doesn't always work if they're moving too fast.", function (on)
    AIM_LEGITSILENT = on
end, true)

menuToggle(silentAimSettings, KER_LANG_TABLE[54], {"silentcheckveh"}, "Will check if the selected player is in a vehicle. If they are in a vehicle, and HEAD isn't selected, will target their head automatically to increase chances of killing.", function (on)
    AIM_HEADVEH = on
end)

menuToggle(silentAimSettings, KER_LANG_TABLE[55], {"silentnpc"}, "Toggle this to ONLY silent aimbot NPCs. Toggle off for ONLY players.", function (on)
    AIM_NPCS = on
end)

menu.divider(silentAimSettings, "-----------------")

menuToggle(silentAimSettings, KER_LANG_TABLE[56], {"silentaimhead", "silenthead", "saimhead"}, "Makes the aimbot target the head. Probably doesn't look legitimate, but ok.", function(on)
    AIM_Head = on
end)

menuToggle(silentAimSettings, KER_LANG_TABLE[57], {"silentaimspine2", "silentspine2", "saimspine2"}, "Makes the aimbot target the body, also known as spine2.", function(on)
    AIM_Spine2 = on
end)

menuToggle(silentAimSettings, KER_LANG_TABLE[58], {"silentaimpelvis", "silentpelvis", "saimpelvis"}, "Makes the aimbot target the pelvis.", function (on)
    AIM_Pelvis = on
end)

menuToggle(silentAimSettings, KER_LANG_TABLE[59], {"silentaimtoe", "silenttoe", "saimtoe"}, "Makes the aimbot target the toe, otherwise known as toe0", function (on)
    AIM_Toe0 = on
end)

menuToggle(silentAimSettings, KER_LANG_TABLE[60], {"silentaimhand", "silenthand", "saimhand"}, "Makes the aimbot target the hand, otherwise known as R_Hand", function (on)
    AIM_RHand = on
end)
    ...
    if on then
        ...
        while MISL_AIM do
            ...
            if missile_settings.multitarget then
                p = GetClosestPlayerWithRange_Whitelist_DisallowEntities(missile_settings.radius, MISSILE_ENTITY_TABLE)
            elseif missile_settings.multiped then
                p = GetClosestNonPlayerPedWithRange_DisallowedEntities(missile_settings.radius, MISSILE_ENTITY_TABLE)
            elseif not missile_settings.multitarget then
                p = GetClosestPlayerWithRange_Whitelist(missile_settings.radius)
            end
...
menuAction(helperFeatures, "Teleport safe codes", {}, "", function()
    local objTable = entities.get_all_objects_as_pointers()
