...
menu.toggle(pvphelp, KER_LANG_TABLE[66], {"rpgaim"}, "You heard me. Only the REGULAR RPG, not the homing one. Works on vehicles as well, such as Lazer or Buzzard. No guarantees, though!", function (on)
    if on then
        MISL_AIM = true
...
menu.slider(rpgsettings, KER_LANG_TABLE[70], {"msl_spd_mult"}, "Multiplier for speed. Default is 100, it's good.", 1, 10000, 100, 100, function (value)
    MISL_SPD = value
end)
...
local function playerActionsSetup(pid) --set up player actions (necessary for each PID)
    ...
    menuToggleLoop(ptossf, KER_LANG_TABLE[174], {}, "Gets the coodinates that you want them to go to from your shot.", function()
    ...
    menuAction(ptossf, KER_LANG_TABLE[175], {}, "", function ()
        util.toast(Want.x .. " " .. Want.y .. " " .. Want.z)
    end)

    menuAction(ptossf, KER_LANG_TABLE[176], {}, "", function ()
        Want.x = 0
        Want.y = 0
        Want.z = 0
    end)
    ...
    menuToggleLoop(ptossf, KER_LANG_TABLE[177], {"bettertoss"}, "IT'S FINALLY HERE!.", function ()
